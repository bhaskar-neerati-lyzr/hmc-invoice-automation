...
import lyzr_client
from pdf_render import render_pdf_to_jpegs
...
async def pages_from_upload(file: UploadFile) -> list[bytes]:
    """Turn one uploaded file into an ordered list of page-JPEGs: a PDF
    becomes one JPEG per page, an image becomes a single-element list."""
    if file.content_type not in ALLOWED_CONTENT_TYPES:
        raise HTTPException(400, f"Unsupported file type: {file.content_type}. Use PNG, JPG, or PDF.")
